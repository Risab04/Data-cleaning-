...
from layoffs_staging)
select * , row_number() over() as count from duplicate_cte where row_num > 1;
...
update layoffs_staging2 
set country = 'US' where country like 'United States%';
...
join layoffs_staging2 t2 
 ...
 update layoffs_staging2 set industry = null   -- getting rid of blank 
 where industry = '';
...
select * from layoffs_staging2 where company = 'airbnb';  -- no null value in column industry 

-- remove any col and row if needed 
select * from layoffs_staging2 where total_laid_off is null and 
percentage_laid_off is null;
